...
순서가 중요하다. 보장을 먼저 정하면 성능과 실패의 모양도 상당 부분 따라 정해진다. `PACELC`가 이 것을 이야기한다. 일관성을 강하게 잡으면 지연이 따라 붙는다.
...
4. p99 지연 예산은 얼마인가. `PACELC`의 `E(else)`를 어디로 둘지 결정한다. 가령 예산이 1ms라면 리전(region) 간 동기 합의는 불가능하다.
...
코디네이션에는 늘 비용이 따라온다. 지연이거나, 가용성이거나, 둘 다거나. "공짜 강한 일관성"이라는 표현은 마케팅에서나 이야기 된다.

---

## 2. 네 축 프레임워크

흔히 6축(속도 / 진실 / 규모 / 형태 / 실패 / 운영)으로 쪼개기도 한다. 기억하기는 좋지만 축끼리 꽤 겹친다. 속도는 진실을 얼마나 강하게 잡느냐에 따라 달라지고, 규모는 대개 성능(performance envelope) 안에서 드러난다. 운영(Ops) 비용도 독립된 속성이라기보다 보장과 워크로드, 실패 선택의 결과로 따라온다. 그래서 이 글에서는 네 축으로 압축한다.
...
| 인과 일관성 (causal consistency) | 인과 관계가 있는 연산들의 순서만 보존한다.                                  | Lamport 1978 |
...
지연은 평균이 아니라 분포로 본다. Dean과 Barroso의 `The Tail at Scale`에서 짚었듯이, 팬아웃(fan-out) 아키텍처에서는 꼬리 지연(tail latency)이 시스템 전체 지연을 지배한다. 100개로 팬아웃하면 시스템 지연은 사실상 컴포넌트의 p99이 되고, 1000개로 팬아웃하면 p999가 된다. 그래서 컴포넌트를 평가할 때 p50만 빠른 컴포넌트는 함정이다. p50, p99, p999를 따로 본다.

처리량(throughput) 천장은 `Little's Law`로 추산한다.
...
- $L$: 평균 시스템 내 요청 수(in-flight requests)
- $\lambda$: 처리율(throughput)
- $W$: 평균 응답시간
...
코디네이션 비용은 한 연산이 몇 개 노드의 합의를 요구하는가의 문제다. 이 값이 지연 하한을 직접 결정한다. `PACELC`의 `E(else)`를 만들어 내는 메커니즘이다.
...
새 컴포넌트를 만나면 늘 이 두 질문을 던진다. 1) 한 쓰기에 몇 노드가 동기적으로 참여하는가. 2) 그 노드들 사이 RTT는 얼마인가. 이 두 숫자가 사실상 지연 하한을 결정한다. 어떤 최적화도 이 하한 아래로는 못 내려간다. §1.1에서 본 광속 한계가 허락하지 않는다.
...
실패 모델은 해당 컴포넌트가 어떤 종류의 장애를 전제로 설계되었으며, 그 중 어디까지를 정상 동작 범위로 간주하는지를 정의한다.
...
Harvest와 Yield (Fox & Brewer, HotOS 1999)는 CAP의 이분법을 완화한 관점이다.
...
첫째, "셋 중 둘만 고르라"는 표현은 정확하지 않다. `P(Partition)`는 현실이지 선택지가 아니다. 실제 선택은 파티션이 발생했을 때 C와 A 중 하나다. 파티션이 없을 때는 둘 다 가능하다.

둘째, "`C(Consistency)`는 일반적인 의미의 일관성"이 아니다. CAP의 C는 선형화 가능성(Linearizability)이다. 스냅샷 격리나 결과적 일관성 같은 다른 의미의 "일관성"은 CAP의 영역 밖이다.

셋째, "`A(Availability)`는 SLA 가용성"도 아니다. CAP의 A는 살아있는 모든 노드가 응답해야 한다. 일부 노드가 응답하지 않아도 SLA를 만족하는 시스템은 CAP의 A를 위반하지만 실용적으로는 가용하다.

CAP은 강력한 단순화지만 실무 도구로는 거칠다. 다음의 `PACELC`가 더 유용하다.
...
- Fractal tree, Bε-tree(B-epsilon-tree). B-tree와 LSM 사이의 균형이다. TokuDB.
...
각 카드는 보장 / 성능 / 워크로드 / 실패의 네 축으로 정리한다. 숫자는 벤치마크가 아니라 대략적인 `order-of-magnitude`다. 정확한 값은 워크로드와 하드웨어, 클라우드, 설정, 클라이언트 라이브러리에 따라 크게 달라진다. 여기서는 의사결정을 위한 자릿수 감각만 본다.
