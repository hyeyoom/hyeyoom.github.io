...
**핵심 주장.** Speed/Truth/Scale은 독립축이 아니다. **Invariant를 먼저 고르면, Performance와 Failure는 그 결과로 강제된다.** PACELC가 이걸 정확히 말한다 — consistency를 강하게 잡으면 latency가 따라온다.
...
4. p99 latency 예산은? → PACELC의 E를 강제. 1ms 예산이면 cross-region 동기 합의 불가
...
- 어떤 자료구조도 **읽기, 쓰기, 메모리** 셋 다를 동시에 최소화할 수 없다 — 이게 RUM Conjecture (§2.3).
...
- **Quorum 정리** — N개 replica에서 read-after-write를 보장하려면 |R| + |W| > N. Dynamo, Cassandra의 ONE/QUORUM/ALL 옵션을 이해하는 기본 모델이다.
- **합의의 라운드 수** — Paxos/Raft 정상 경로는 최소 2 message delay. 이게 cross-region 강일관 쓰기가 100ms 안에 못 끝나는 이유.
...
흔히 제안되는 6축 — Speed / Truth / Scale / Shape / Failure / Ops — 은 실용적이지만 orthogonality가 떨어진다. Speed와 Truth는 PACELC가 말하듯 *서로의 함수*다. Capacity는 Performance envelope의 일부다. Ops cost는 대부분 다른 축의 *결과*다. 그래서 다음 4축으로 재정리한다.
...
**(b) Throughput 천장.** **Little's Law**: L = λ·W. concurrency = arrival rate × residence time. 의미:
- 100 connection 풀 + 요청당 10ms = 10,000 req/s
- 목표 10k req/s + 100 worker = 요청당 10ms 이내 끝나야 함
- λ가 천장에 가까워질수록 **W가 폭발한다** (queueing theory, M/M/1: W = 1/(μ−λ))
...
- α = 직렬화 계수 (공유 자원 contention)
- β = coherency 계수 (노드 간 cross-talk, 캐시 동기화 등)
...
**(d) Coordination cost.** 한 op이 *몇 개 노드의 합의*를 필요로 하는가? 이게 latency 하한을 *직접* 결정한다 — PACELC의 E를 산출하는 메커니즘.
...
CAP "셋 중 하나 포기"처럼 두루뭉술한 정리는 실무 판단에 도움이 안 된다. 정확한 statement로 내장하자.
...
CAP의 진정한 확장. 핵심 통찰: **partition이 없을 때도 consistency와 latency는 trade된다.** consistency를 강하게 잡으려면 더 많은 노드와의 합의를 기다려야 하니까.
...
아직 conjecture지만 강력한 디자인 가이드다. 새 인덱스/스토리지 엔진이 등장하면 *어떤 두 개에 강하고 어디에 비용을 떠넘기는지* 묻는 게 첫 질문.
...
> L = λ · W
> (system 내 평균 요청 수) = (arrival rate) × (평균 residence time)
...
**확장.** queueing theory M/M/1에서 평균 잔류 시간 W = 1/(μ−λ). 사용률 ρ = λ/μ가 1에 가까워지면 W는 hyperbolic하게 증가한다. 이게 "DB CPU 80% 넘어가면 latency가 무너진다"의 수학적 정체.
...
**핵심 인사이트.** β > 0 이면 어떤 *최적 노드 수* N* = √((1−α)/β) 가 존재하고, 그 이상에서는 throughput이 *감소*한다 (retrograde scaling).

**실무 적용.** 부하 테스트 결과 (N, throughput) 점들을 USL로 fit하면 α, β가 나옴. β가 0에 가깝지 않으면 아키텍처에 본질적 cross-talk가 있다는 신호 (distributed lock, 글로벌 카운터, cache invalidation 폭주 등). 더 많이 넣어서 해결이 안 되는 클래스의 문제.
