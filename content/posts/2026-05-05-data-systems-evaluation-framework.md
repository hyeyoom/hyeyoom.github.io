...
| LSM tree | 중간 (컴팩션 꼬리) | 낮음 | 낮음 (쓰기 증폭은 큼) | 쓰기 위주 + range |
...
| MongoDB (default) | PA / EC | 파티션 시 A, 평상시 C (튜닝 가능) |
...
- LSM tree (RocksDB, Cassandra, ScyllaDB). U가 좋고 R/M에 비용이 든다. 컴팩션이 R의 꼬리를 만들고 쓰기 증폭(write amplification)이 M에 영향을 준다.
...
각 카드는 보장 / 성능 / 워크로드 / 실패의 네 축으로 정리한다. 숫자는 대략적인 자릿수다. 정확한 값은 워크로드와 하드웨어, 설정에 따라 다르지만 멘탈 모델 차원에서는 자릿수 감각이 더 중요하다.

---

### Redis

- 보장. 단일 키 op는 단일 스레드(single-thread)라서 primary 기준 선형화 가능하다. 복제는 비동기가 기본이라 복제본은 stale일 수 있다. 내구성은 옵션이다. RDB(스냅샷), AOF(`appendfsync everysec`이면 최대 1초 손실, `always`면 매 쓰기마다 fsync로 ms 수준 비용). 클러스터(cluster) 모드는 샤드 간(cross-shard) 트랜잭션을 지원하지 않는다.
...
- 보장. 문서 단위 원자성이 기본이다. multi-document 트랜잭션도 지원하지만 트랜잭션의 read/write concern과 primary routing 조건을 이해해야 한다. default read concern이 `local`이라 롤백될 수 있는 데이터를 읽을 수 있고, 일반 replica set의 implicit default write concern은 대체로 `w: majority`다(arbiter 구성은 예외). 인과 일관성은 session + majority read/write concern 조합에서 의미가 있다.
