...
> p50, p95, p99는 분포에서 특정 백분위 지점의 값이다.
> 지연(latency), 응답 시간, 큐(queue) 대기 시간, DB 쿼리(query) 시간, 외부 API 호출 시간처럼 "시간이 얼마나 걸렸는가"를 볼 때 자주 쓴다.
>
...
p50은 일반적인 코드 경로가 무거운지, 캐시(cache)가 잘 먹는지, DB 인덱스가 잘 타는지 볼 때 유용하다.
하지만 p50만 보면 장애를 놓친다.
...
실무에서는 이 꼬리가 중요하다.
사용자는 "평균 시스템"을 경험하지 않고, 자기가 걸린 그 요청 하나를 경험하기 때문이다.
...
인기 상품 하나에 요청이 몰리면 로우 락(row lock) 대기가 발생한다.
p50은 멀쩡한데 p99는 급격히 증가한다.

이 경우 CPU를 늘려도 해결이 안 된다.
대기 중인 건 CPU가 아니라 락이기 때문이다.
...
그러면 일부 요청은 DB 커넥션을 얻기 위해 기다린다.
실제 쿼리 시간은 20ms인데 API 응답 시간은 2초가 될 수 있다.
...
반대로 말하면 약 9.6%의 요청에서는 적어도 하나의 API가 꼬리에 걸릴 수 있다.
...
대부분의 관측 시스템은 히스토그램(histogram)이나 summary 형태로 근사 계산한다.
...
요청이 들어올 때마다 해당 버킷 카운트를 증가시킨다.
나중에 버킷 분포를 보고 p95/p99를 추정한다.
...
버킷이 이렇게만 있으면 p95가 300ms인지 900ms인지 구분하기 어렵다.
둘 다 `<= 1s` 버킷에 들어가기 때문이다.
...
Little's Law 관점에서도 큐가 차면 잔류 시간(residence time)이 늘어난다.[^littles-law]
...
실패 요청이 타임아웃으로 느리게 실패하면 p99를 올린다.
...
트래픽이 적은 API는 p99보다 p95, max, timeout count, 트레이스 샘플을 보는 게 더 낫다.
...
p99 원인이 CPU 포화면 서버 증설이 먹힌다.
하지만 원인이 DB 락, slow query, 외부 API, 커넥션 풀 대기, GC pause, 핫 키(hot key), 재시도 폭주, 순차 팬아웃이면 서버만 늘려도 별 효과가 없거나 오히려 악화될 수 있다.
...
"p99가 몇 ms인가?"에서 멈추지 않고, 그 p99가 어떤 워크로드, 관측 구간, 라벨, 측정 방식에서 나온 숫자인지 확인해야 한다.
...
클라이언트가 요청 하나를 보내고 응답을 기다린 뒤 다음 요청을 보내는 방식이라고 하자.
서버가 3초 멈추면, 그 3초 동안 클라이언트도 같이 멈춰서 새 요청을 보내지 않는다.

그러면 실제 프로덕션에서는 큐가 쌓였을 상황인데, 테스트 결과에는 "3초짜리 느린 요청 하나"만 기록된다.
꼬리 지연이 과소 측정된다.
...
성능의 본질은 부하가 올라가도 꼬리를 통제할 수 있는가다.
...
이 관점은 앞 글의 성능(Performance envelope)과 이어진다.
성능은 단일 숫자가 아니라 응답 시간 분포, 처리량(throughput) 천장, 용량, 코디네이션 비용이 함께 만드는 영역이다.
...
이게 진짜 성능이다.
앞의 글에서 성능을 응답 시간 분포, 처리량 천장, 용량, 코디네이션 비용으로 나눈 이유도 여기에 있다.
...
분포를 기준으로 정상 경로와 병목 경로를 분리해서 보는 렌즈다.
...
[^tail-at-scale]: Dean & Barroso의 "The Tail at Scale"은 팬아웃 시스템에서 꼬리 지연이 어떻게 사용자 경험을 지배하는지 설명하는 대표적인 글이다.

[^littles-law]: Little's Law는 $L = \lambda W$로 표현된다. 시스템 안의 평균 요청 수($L$)는 도착률($\lambda$)과 평균 잔류 시간($W$)의 곱이다. 큐가 길어지면 요청이 시스템 안에 머무는 시간도 늘어난다.
